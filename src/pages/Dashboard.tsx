    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Bem-vindo de volta ao seu ateliê, {settings.brandName || 'Artesão'}.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="hover:shadow-md transition-shadow duration-200 border-l-4 border-l-blue-500">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-500 mb-1">Matérias-Primas</p>
              <h3 className="text-3xl font-bold text-gray-900">{totalMaterials}</h3>
            </div>
        ...
        <Card className="hover:shadow-md transition-shadow duration-200 border-l-4 border-l-emerald-500">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              ...
              <h3 className="text-3xl font-bold text-gray-900">{avgMargin.toFixed(1)}%</h3>
            </div>
        ...
        <Card className="hover:shadow-md transition-shadow duration-200 border-l-4 border-l-amber-500">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-500 mb-1">Potencial (1 lote)</p>
              <h3 className="text-2xl font-bold text-gray-900">{formatCurrency(potentialRevenue)}</h3>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card className="lg:col-span-2 shadow-sm">
          <CardHeader className="border-b border-gray-50 pb-4">
            <CardTitle className="text-lg font-semibold text-gray-800">Top 5 Produtos Mais Lucrativos (R$/un)</CardTitle>
          </CardHeader>
              ...
              {lowMarginProducts.length > 0 ? (
                <div className="space-y-3">
                  {lowMarginProducts.slice(0, 4).map(p => (
                    <div key={p.id} className="flex items-center justify-between p-3 bg-amber-50/50 hover:bg-amber-50 transition-colors rounded-lg border border-amber-100 group cursor-pointer">
                      <span className="font-medium text-amber-900 truncate max-w-[140px]">{p.name}</span>
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-amber-700 bg-amber-100 px-2 py-0.5 rounded">
                ...
                <div className="h-full flex flex-col items-center justify-center text-center py-8">
                  ...
                  <h4 className="font-medium text-gray-900">Tudo certo!</h4>
                  <p className="text-sm text-gray-500 mt-1">Seus produtos estão com boa margem de lucro.</p>
                </div>
              )}
            </CardContent>
          </Card>
